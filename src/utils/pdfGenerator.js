...
export const generateCertificatePDF = (certificateData) => {
  ...
  const { name, nit, year, type, details, period } = certificateData;
  ...
  const typeTitle = type.toUpperCase() === 'RETEIVA' ? 'RETENCION EN EL IVA' : 
                   type.toUpperCase() === 'RETEFUENTE' ? 'RETENCION EN LA FUENTE' : type.toUpperCase();
  doc.text(`CERTIFICADO DE ${typeTitle}`, 110, 18, { align: 'center' });
  
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  const dates = getPeriodDates(period, year);
  const dateText = type.toUpperCase() === 'RETEIVA' 
  ...
  doc.setFont('helvetica', 'normal');
  doc.text('Nit: 900973932-9', 45, 42);
  doc.text('CR 16 CALLE 77 BODEGA 3 LA ROMELIA LOTE PARQ.', 45, 46);
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.text('CONCEPTO RETENCIÓN', 45, 71, { align: 'center' });
  doc.text('MONTO SUJETO A RETENCIÓN', 125, 71, { align: 'center' });
  doc.text('VALOR TOTAL RETENCIÓN', 175, 71, { align: 'center' });
  ...
  details.forEach(d => {
    ...
    doc.text(formatCurrency(d.baseAmount), 125, currentY, { align: 'center' });
    doc.text(formatCurrency(d.retainedAmount), 175, currentY, { align: 'center' });
    currentY += 6;
  });
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.text('Ciudad donde se consignó :  PEREIRA - RISARALDA', 13, cityY + 5);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text('JULIANA GUTIERREZ', 46, footerY + 12, { align: 'center' });
  doc.setFontSize(7);
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.text(`Fecha y hora de expedición   ${dateStr} - ${timeStr}`, 198, footerY + 12, { align: 'right' });
